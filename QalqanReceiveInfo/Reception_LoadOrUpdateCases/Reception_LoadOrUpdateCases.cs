...
namespace Qalqan.ReceiveInfo.Examples.Reception_LoadOrUpdateCases
{
    // =====================================================================================
    ...
    public class IntegraRequest
    {
        ...
        public string RequestType { get; set; } = "Reception_LoadOrUpdateCases";

        public LoadReceptionRoomCasesRequest? Reception_LoadOrUpdateCasesRequest { get; set; }
    ...
    /// <summary>Пакет обращений приёмного покоя (Reception_LoadCases / Reception_LoadOrUpdateCases).</summary>
    public class LoadReceptionRoomCasesRequest
    {
        [XmlArray("List")]
        [XmlArrayItem("ReceptionRoomCaseRequest")]
        public List<ReceptionRoomCaseRequest> List { get; set; } = new List<ReceptionRoomCaseRequest>();
    }

    /// <summary>Обращение в приёмный покой (без госпитализации).</summary>
    public class ReceptionRoomCaseRequest
    {
        /// <summary>Идентификатор приёма в МИС (обязателен).</summary>
        ...
        /// <summary>Дата обращения (yyyy-MM-dd).</summary>
        [XmlIgnore]
        public DateTime AppealDate { get; set; }

        [XmlElement("AppealDate")]
        public string AppealDateXml
        {
            get => XmlDateFormats.FormatDate(AppealDate);
            set => AppealDate = XmlDateFormats.ParseDate(value);
        }

        /// <summary>Основной предварительный диагноз (МКБ-10). Обязателен.</summary>
        public string PrimaryPreliminaryDiagnosisCode { get; set; } = string.Empty;

        [XmlArray("Services")]
        [XmlArrayItem("ReceptionRoomServiceItemRequest")]
        public List<ReceptionRoomServiceItemRequest> Services { get; set; } = new List<ReceptionRoomServiceItemRequest>();
    ...
    public class ReceptionRoomServiceItemRequest
    {
    ...
    public class LoadReceptionRoomCasesResponse : LoadResultBase
    {
        [XmlArray("Errors")]
        [XmlArrayItem("ReceptionRoomCaseErrorDto")]
        public List<ReceptionRoomCaseError> Errors { get; set; } = new List<ReceptionRoomCaseError>();
    }

    public class ReceptionRoomCaseError : RecordErrorBase
    {
        public long AdmissionId { get; set; }
        public string? AppealDate { get; set; }
        public string? OrganizationBin { get; set; }
        public string? DoctorIin { get; set; }
        public string? PrimaryPreliminaryDiagnosisCode { get; set; }
    }
    ...
    public LoadReceptionRoomCasesResponse? Reception_LoadOrUpdateCasesResponse { get; set; }

    /// <summary>Блок результата (есть только при StatusCode = 200).</summary>
    public LoadReceptionRoomCasesResponse? Result => Reception_LoadOrUpdateCasesResponse;
    ...
    public static class Example
    {
        ...
        {
            Reception_LoadOrUpdateCasesRequest = new LoadReceptionRoomCasesRequest
            {
                List =
                {
                    new ReceptionRoomCaseRequest
                    {
                        AdmissionId = 5011,
                        ...
                        AppealDate = new DateTime(2026, 4, 10),
                        PrimaryPreliminaryDiagnosisCode = "J06.9",
                        Services =
                        {
                            new ReceptionRoomServiceItemRequest { ServiceCode = "B01.047.001", Quantity = 1, Date = new DateTime(2026, 4, 10) },
                            new ReceptionRoomServiceItemRequest { ServiceCode = "B03.016.003", Quantity = 2, Date = new DateTime(2026, 4, 10) },
                        },
                        BgFinanceSource = FinanceSource.Osms                     // 1
                    }
                }
            }
        };
